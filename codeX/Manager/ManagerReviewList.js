...
import { managerGetReviews, managerReviewGetProducts } from "../../actions/managerAction";
import ManagerSideBar from "./ManagerSideBar";


export default function ManagerReviewList() {

  const { reviews = [], products = [], loading = true, error } = useSelector(state => state.managerState);
  ...
  useEffect(() => {
    ...
    if (debouncekeyword.length >= 2) {
      dispatch(managerReviewGetProducts(debouncekeyword));
    }
    if (debouncekeyword.length === 0) {
      dispatch(managerReviewGetProducts(""));
    }
              ...
              >
                {products.map((p, idx) => (
                  <Card
                    ...
                    onClick={() => {
                      setProductId(p._id);
                      dispatch(managerGetReviews(p._id));
                    }}
                  >
