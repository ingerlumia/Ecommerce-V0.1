...
import { sellerOrdersList } from "../../actions/sellerAction";
import { sellerclearError } from "../../slices/sellerSlice";
import { FaEdit } from "react-icons/fa";

export default function SellerOrderList() {
    const { userOrders = [], loading = true, error } = useSelector(state => state.sellerState);
                                        ...
                                        <table className="table table-striped align-middle">
                                            ...
                                            <tbody>
                                                {userOrders.map((order, idx) => {
                                                   ...
                                                   return( <tr key={idx}>
                                                        <td><Link to={`/product/${order.orderItems[0]?.product}`}>{order.orderItems[0]?.product}</Link></td>
