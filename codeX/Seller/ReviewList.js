...
import { Container, Row, Col, Card, Button, Table, FormControl, ListGroup, Badge, Form } from "react-bootstrap";
import { FaTrash, FaStar, FaEdit } from "react-icons/fa";
import { adminGetProducts, deleteReviews, getProducts, getReviews, reviewGetProducts } from "../../actions/productsActions";
import { clearError, clearIsReviewDeleted } from "../../slices/productSlice";

export default function ReviewList() {
  
  const { reviews = [], products = [], isReviewDeleted, loading = true, error } = useSelector(state => state.productState);
  ...
  useEffect(() => {
    ...
    if (debouncekeyword.length >= 2) {
      dispatch(reviewGetProducts(debouncekeyword));
    }
    if (debouncekeyword.length === 0) {
      dispatch(reviewGetProducts(""));
    }
